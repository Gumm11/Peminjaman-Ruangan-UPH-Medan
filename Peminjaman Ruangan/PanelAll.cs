...
{
    public partial class PanelAll : UserControl
    {
        public PanelAll()
        {
            InitializeComponent();
        ...
        private void UpdateRejectionReason(int formId)
        {
            frmReject2 rejectForm = new frmReject2(formId);
            rejectForm.Show();
            rejectForm.Close();
        }
        ...
        private void BtnCancel_Click(object sender, EventArgs e)
        {
            ...
            while (parentControl != null && !(parentControl is PanelAll))
            {
                parentControl = parentControl.Parent;
            }

            // Check if a Requests instance was found
            if (parentControl is PanelAll)
            {
                // Now you can cast it safely
                PanelAll clickedForm = (PanelAll)parentControl;
                ...
                {
                    // User clicked OK, open frmReject2 for cancel reason input
                    frmReject2 rejectForm = new frmReject2(formID);
                    rejectForm.ShowDialog();  // Show as a modal dialog

                    // Update the Form_Status in the database
                    UpdateFormStatus(formID, "inactive"); // Implement the method to update the status
                    UpdateRejectionReason(formID);

                    // Close the parent form
                    frmAllForm parentForm = (frmAllForm)this.ParentForm;
                    parentForm.Close();

                    // Create a new instance of the parent form
                    frmAllForm newForm = new frmAllForm();
                    newForm.Show();
                }
